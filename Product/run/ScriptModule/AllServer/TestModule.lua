...
function TestModule.Init()
	...
	--io.write("AppName:" .. pPluginManager:GetAppName() .. "\n");
	
	--io.write("Addr of pPluginManager " .. tostring(pPluginManager) .. "\n");

	--local pKernelModule = pPluginManager:FindKernelModule();
	--io.write("Addr of NFCKernelModule " .. tostring(pKernelModule) .. "\n");

	--local pClassModule = pPluginManager:FindClassModule();
	--io.write("Addr of NFCClassModule " .. tostring(pClassModule) .. "\n");

	--io.write("Addr of NFCElementModule " .. tostring(pElementModule) .. "\n");

	--pLuaScriptModule:AddClassCallBack("Player", "TestModule.OnClassCommonEvent");
end

function TestModule.AfterInit()
	--io.write("TestModule AfterInit!\n");
	
	--local pKernelModule = pPluginManager:FindKernelModule();

	--local pObject = pKernelModule:CreateObject(NFGUID(), "Player", "0", NFDataList());
	--pObject = nil
	--local OID = pObject:Self();
	--pKernelModule:DestroySelf(OID);
	--io.write("pKernelModule:CreateObject:" .. OID:ToString() .. "\n")
	--property callback
