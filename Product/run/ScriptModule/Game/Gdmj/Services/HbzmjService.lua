...
function HbzmjService.DoWork(tItem) --传入的是TdhStruct类型的Item
	if tItem.m_tInfo.status == g_gdmjStatus.status_ready then
		--牌桌处于等待的状态，如果人满了就开始
		HbzmjService.TableReady(tItem)
	...
	elseif tItem.m_tInfo.status == g_gdmjStatus.status_dissolve then
		...
		if tItem.m_tInfo.timemark <= 0 then
			--这里开始下发数据
			HbzmjService.PlayCountDissolve(tItem)
		end
...
function HbzmjService.TableReady(tItem)  --牌桌处于等待状态的函数
	local isReady = true
...
function HbzmjService.InitPublicPoker(tItem)
	...
	if (tItem.m_maxUser == 3 and tItem.m_vipRoomInfo.queyimen == 1) or tItem.m_maxUser == 2 then
		--二人，三人情况下不要万字
		for i = 1,4 do
			for j = 2,5 do
	...
	local lenNum = #tItem.m_tInfo.publicpoker
...
function HbzmjService.StartTest(tItem)
...
function HbzmjService.GameStart(tItem)

	--初始化公共牌
	HbzmjService.InitPublicPoker(tItem)
	...
	local function HbzBuHua(tItem, pos)
		--开始补花了
	...
	lenPub = #tItem.m_tInfo.publicpoker
		
end

function HbzmjService.TablePlay(tItem,currPos)
	
	if tItem.m_tInfo.userstate == g_gdmjUserState.state_waiting then
		tItem.m_isModify = true
		
		HbzmjService.CheckMoving(tItem, false)
...
function HbzmjService.CheckMoving(tItem,isLog)
	if true == isLog then
	...
	if #currPosList == 0 then
		LogFile("error", "HbzmjService.CheckMoving actchairid is nil")
		return nil
	end
...
function HbzmjService.PlayingAction(tItem,cgmsg, gcmsg)
	...
	if cgmsg.actiontype == g_gdmjAction.type_play then
		...
		HbzmjService.DoPlay(tItem,cgmsg,gcmsg)
	...
	elseif cgmsg.actiontype == g_gdmjAction.type_guo then
		isWin = HbzmjService.DoGuo(tItem, cgmsg, gcmsg)  --这里过的时候，必须要在这里返回
	elseif cgmsg.actiontype == g_gdmjAction.type_chi then
		HbzmjService.DoChi(tItem, cgmsg, gcmsg)
	elseif cgmsg.actiontype == g_gdmjAction.type_peng then
		HbzmjService.DoPeng(tItem, cgmsg, gcmsg)
		
	elseif cgmsg.actiontype == g_gdmjAction.type_gang then
		HbzmjService.DoGang(tItem, cgmsg, gcmsg)
		GdmjWork.CheckHuList(tItem, cgmsg.actchairid)   --检查听牌列表
	...
	elseif cgmsg.actiontype == g_gdmjAction.type_buhua then
		--这个是补花
		HbzmjService.DoBuHua(tItem, cgmsg, gcmsg)
		GdmjWork.CheckHuList(tItem, cgmsg.actchairid)   --检查听牌列表
	end
	...
	if isWin == true then
		HbzmjService.DoWin(tItem, cgmsg.actchairid, cgmsg.actchairid)
		isEnd = true
	else
		tItem.m_userList[cgmsg.actchairid].playstate = g_gdmjPlayerState.play_waiting
		if tItem.m_tInfo.beingpoker == 1 then
			--只有发牌的情况下会去检查流局
			isEnd = HbzmjService.CheckLiuJu(tItem)
		end
	end
...
function HbzmjService.DoPlay(tItem, cgmsg, gcmsg)
	--这个是打牌的操作
	...
	local chiHu = HbzmjService.NewCheckChiHu(tItem,cgmsg.actpokerid, cgmsg.actchairid,false)  --这里检查吃胡的时候，在自己本地检查	
	...
	if chiHu > 0 then
		...
		HbzmjService.CheckGenZhuang(tItem,cgmsg)   --检查跟庄
	end
end

function HbzmjService.DoGuo(tItem,cgmsg,gcmsg)
	...
	if tItem.m_tInfo.nextinfo.tarchairid == 0 then
		...
		if prevType == g_gdmjAction.type_qiangganghu then
			...
			--过了抢杠胡了
			tItem.m_tInfo.chihulist:remove(1)
			local chiHu = HbzmjService.NewCheckChiHu2(tItem)
		...
		elseif prevType == g_gdmjAction.type_chihu then
			...
			if chiHu > 0 then
				...
				if nextPos ~= 0 then
					local tempList = HbzmjService.GetChiHuList(tItem, cgmsg.actpokerid, cgmsg.tarchairid)
...
function HbzmjService.DoPeng(tItem, cgmsg, gcmsg)
...
function HbzmjService.DoGang(tItem, cgmsg, gcmsg)
	...
	if cgmsg.tarchairid == 0 then
		...
		for k,v in ipairs(tItem.m_userList[cgmsg.actchairid].mjpokerlist) do
			if v.pokerlist[1] == cgmsg.actpokerid and v.pokertype == g_gdmjAction.type_peng then
				qgHuChair = HbzmjService.NewCheckChiHu(tItem, cgmsg.actpokerid, cgmsg.actchairid, true)
				break
			end
		end
	else
...
function HbzmjService.DoGenZhuang(tItem, cgmsg, gcmsg)
	
end

function HbzmjService.DoBuHua(tItem, cgmsg, gcmsg)
	--已经有人赢了
	--tItem.m_tInfo.status = g_gdmjStatus.status_counting   --进入结算的时间
	--tItem.m_tInfo.timemark = g_gdmjTime.counting_time
	tItem.m_tInfo.prevpos = cgmsg.actpokerid
	GdmjWork.DelPokerFromHand(tItem.m_userList[cgmsg.actchairid],cgmsg.actpokerid)	
	tItem.m_tInfo.beingpoker = 1   --杠完后需要发牌的
	tItem.m_nextInfo.actchairid[cgmsg.actchairid] = cgmsg.actchairid
...
function HbzmjService.DoWin(tItem, pos, pokerID)
	--已经有人赢了
	tItem.m_tInfo.status = g_gdmjStatus.status_counting   --进入结算的时间
	tItem.m_tInfo.timemark = g_gdmjTime.counting_time
end

function HbzmjService.DoChi(tItem, cgmsg, gcmsg)
...
function HbzmjService.CheckLiuJu(tItem)
...
function HbzmjService.PlayCount(tItem)

	local len = #tItem.m_tInfo.winchairlist
	
	if #tItem.m_tInfo.winchairlist == 0 then
		HbzmjService.PlayCountLiuJu(tItem)
	elseif #tItem.m_tInfo.winchairlist == 1 then
		HbzmjService.PlayCountWin(tItem)
	else
		HbzmjService.PlayCountWinMore(tItem)
	end
...
function HbzmjService.PlayCountLiuJu(tItem)
	...
	for i = 1,tItem.m_maxUser do
		...
		item.hunum = 0
		item.winjetton = item.gangjetton
...
function HbzmjService.PlayCountDissolve(tItem)
...
function HbzmjService.PlayCountWin(tItem)
	...
	local maBeiNum = 1             --这里的马是跟到对对胡的，所以需要另外算
	...
	local GangBaoJiaFen = 0   --杠爆全包

	local isMenQing = false
	local mingQiangBei = 1          --如果是明杠抢杠胡，就加倍
	...
	if tItem.m_tInfo.wintype == g_gdmjAction.type_qiangganghu then
		...
		for k,v in ipairs(tItem.m_userList[winChairID].handpoker) do
			if v == pokerID then
				mingQiangBei = mingQiangBei + 1
			end
		end
		if mingQiangBei >= 3 then
			mingQiangBei = 2
			table.insert(desList[winChairID],"抢明杠胡翻倍")
		else
			table.insert(desList[winChairID],"抢杠胡")
			mingQiangBei = 1
		end
	...
	elseif tItem.m_tInfo.wintype == g_gdmjAction.type_hu then  --这个是自摸胡
		...
		--表示是杠爆的，在检查是否选择了杠包全包
		if tItem.m_tInfo.prevpos > 60 then
			if tItem.m_vipRoomInfo.bhhpjiafen == 1 then
				GangBaoJiaFen = 3  --家底分
			end
			table.insert(desList[winChairID],"补花胡牌")   --自摸
		elseif tItem.m_tInfo.prevpos > 0 then
			--杠爆加底分
			GangBaoJiaFen = 3  --家底分
			table.insert(desList[winChairID],"杠爆")   --自摸
		end				
	end	
	...
	if tItem.m_tInfo.wintype == g_gdmjAction.type_qiangganghu then
		if tItem.m_vipRoomInfo.qgmaima == 0 then
			--抢杠胡买马没选的情况下，抢杠胡不买码
			maNum  = 0
		end
	end
	...
	maBeiNum = doubleAccount > 9 and 9 or doubleAccount
	

	doubleAccount = doubleAccount*tItem.m_tInfo.pourjetton*mingQiangBei + GangBaoJiaFen
	local countJetton = doubleAccount + maList[winChairID]*maBeiNum
	
	if tItem.m_vipRoomInfo.genhua == 1 then
		--跟花的情况
		countJetton = countJetton + huaNum*(maList[winChairID] + 1)
	else
		countJetton = countJetton + huaNum
	end
...
function HbzmjService.PlayCountWinMore(tItem)
	local gcAccount = msg_gdmj_pb.gcgdmjaccount()
...
function HbzmjService.NewCheckChiHu(tItem,actPokerID,actChairID,isQiangGang)
	...
	for i = 1,#userIDList do
		...
		for k,v in ipairs(tItem.m_userList[ userIDList[i] ].tinglist) do
			if tItem.m_userList[ userIDList[i] ].guohu == 0 and v == actPokerID then
				...
				if isQiangGang == true or tItem.m_userList[ userIDList[i] ].tingtype[k] ~= g_gdmjWinType.type_hzz_jihu then
...
function HbzmjService.NewCheckChiHu2(tItem)
...
function HbzmjService.GetChiHuList(tItem,actPokerID,actChairID)
...
function HbzmjService.ShiSanYaoQiangAnGang(tItem,actPokerID,actChairID,startPos)
	local userIDList = GdmjWork.GetCheckList(actChairID, tItem.m_maxUser) --默认是四个玩家
	--先把需要检查的玩家找出来
	local beginPos = 5
...
function HbzmjService.CheckGenZhuang(tItem, cgmsg)
