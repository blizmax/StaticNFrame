...
function HzmjService.DoWork(tItem) --传入的是TdhStruct类型的Item
	...
	elseif tItem.m_tInfo.status == g_gdmjStatus.status_dissolve then
		...
		if tItem.m_tInfo.timemark <= 0 then
			--这里开始下发数据
			HzmjService.PlayCountDissolve(tItem)
		end
...
function HzmjService.TableReady(tItem)  --牌桌处于等待状态的函数
	local isReady = true
...
function HzmjService.InitPublicPoker(tItem)
...
function HzmjService.StartTest(tItem)
...
function HzmjService.GameStart(tItem)

	--初始化公共牌
	HzmjService.InitPublicPoker(tItem)
...
function HzmjService.TablePlay(tItem,currPos)
	
	if tItem.m_tInfo.userstate == g_gdmjUserState.state_waiting then
		tItem.m_isModify = true
		HzmjService.CheckMoving(tItem)
	else
...
function HzmjService.CheckMoving(tItem)
	
	local currPosList = GdmjWork.GetCurrPosList(tItem.m_tInfo)
	
	if #currPosList == 0 then
		LogFile("error", "HzmjService.CheckMoving actchairid is nil")
		return nil
	end
...
function HzmjService.PlayingAction(tItem,cgmsg, gcmsg)
	...
	if isWin == true then
		HzmjService.DoWin(tItem, cgmsg.actchairid, cgmsg.actchairid)
		isEnd = true
	else
		tItem.m_userList[cgmsg.actchairid].playstate = g_gdmjPlayerState.play_waiting
		if tItem.m_tInfo.beingpoker == 1 then
			--只有发牌的情况下会去检查流局
			isEnd = HzmjService.CheckLiuJu(tItem)
		end
	end
...
function HzmjService.DoPlay(tItem, cgmsg, gcmsg)
	--这个是打牌的操作
...
function HzmjService.DoGuo(tItem,cgmsg,gcmsg)
...
function HzmjService.DoPeng(tItem, cgmsg, gcmsg)
...
function HzmjService.DoGang(tItem, cgmsg, gcmsg)
...
function HzmjService.DoHu(tItem, cgmsg, gcmsg)
	--胡牌
end

function HzmjService.DoQiangGangHu(tItem, cgmsg, gcmsg)
	
end

function HzmjService.DoChiHu(tItem, cgmsg, gcmsg)
	
end

function HzmjService.DoGenZhuang(tItem, cgmsg, gcmsg)
	
end

function HzmjService.DoWin(tItem, pos, pokerID)
	--已经有人赢了
	tItem.m_tInfo.status = g_gdmjStatus.status_counting   --进入结算的时间
	tItem.m_tInfo.timemark = g_gdmjTime.counting_time
end

function HzmjService.CheckLiuJu(tItem)
...
function HzmjService.PlayCount(tItem)

	local len = #tItem.m_tInfo.winchairlist
	
	if #tItem.m_tInfo.winchairlist == 0 then
		HzmjService.PlayCountLiuJu(tItem)
	elseif #tItem.m_tInfo.winchairlist == 1 then
		HzmjService.PlayCountWin(tItem)
	else
		HzmjService.PlayCountWinMore(tItem)
	end
...
function HzmjService.PlayCountLiuJu(tItem)
...
function HzmjService.PlayCountDissolve(tItem)
...
function HzmjService.PlayCountWin(tItem)
	...
	elseif tItem.m_tInfo.wintype == g_gdmjAction.type_hu then  --这个是自摸胡
		...
		if tItem.m_vipRoomInfo.dachuhzjiafen == 1 then   --打出红中翻倍
			local daCount = GdmjHuHelper.DaChuhzjiafen(tItem,winChairID)
			
			if 0 ~= daCount then
				table.insert(desList, "打出红中加分")
				LogFile("hzmj", "daCount="..daCount)
				doubleAccount = doubleAccount + 5*tonumber(daCount)
			end
		end
		
		if tItem.m_vipRoomInfo.gskhjiama == 1 then    --判断是否杠上开花
			if 0 ~= GdmjHuHelper.GskhJiama(tItem,winChairID) then
				table.insert(desList, "杠上开花")
				doubleAccount = doubleAccount + 2
			end			
		end
		
		if 0 ~= GdmjHuHelper.WuhzJiafen(tItem,winChairID) then
			if tItem.m_vipRoomInfo.wuhzjiafen == 1 then    --无红中自摸
				table.insert(desList, "无红中")
...
function HzmjService.PlayCountWinMore(tItem)
	local gcAccount = msg_gdmj_pb.gcgdmjaccount()
