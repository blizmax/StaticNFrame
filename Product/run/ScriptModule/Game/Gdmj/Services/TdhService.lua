...
function TdhService.DoWork(tItem) --传入的是TdhStruct类型的Item
		
	if tItem.m_tInfo.status == g_gdmjStatus.status_ready then
		--牌桌处于等待的状态，如果人满了就开始
		TdhService.TableReady(tItem)
	...
	elseif tItem.m_tInfo.status == g_gdmjStatus.status_end then
		...
		if tItem.m_tInfo.timemark <= 0 then
			...
			GdmjWork.CheckJiFen(tItem, 0)   --在这里检查积分
		end
	elseif tItem.m_tInfo.status == g_gdmjStatus.status_dissolve then
		...
		if tItem.m_tInfo.timemark <= 0 then
			--这里开始下发数据
			TdhService.PlayCountDissolve(tItem)
		end
...
function TdhService.TableReady(tItem)  --牌桌处于等待状态的函数
	local isReady = true
...
function TdhService.InitPublicPoker(tItem)
	...
	tItem.m_tInfo.genzhuang = 0  --重置跟庄
	GdmjWork.InitPubGuiPai(tItem)
	--在这里设置鬼牌
	--在这里还需要确定鬼牌的长度
end

function TdhService.StartTest(tItem)
...
function TdhService.GameStart(tItem)

	--初始化公共牌
	TdhService.InitPublicPoker(tItem)
	...
	--tItem.m_tInfo.bankercount = 0   --这里不能置为0
...
function TdhService.TablePlay(tItem,currPos)
...
function TdhService.CheckMoving(tItem)
	
	local currPosList = GdmjWork.GetCurrPosList(tItem.m_tInfo)
	
	if #currPosList == 0 then
		LogFile("error", "TdhService.CheckMoving actchairid is nil")
		return nil
	end
...
function TdhService.PlayingAction(tItem,cgmsg, gcmsg)
	...
	if isWin == true then
		TdhService.DoWin(tItem, cgmsg.actchairid, cgmsg.actchairid)
		isEnd = true
	else
		tItem.m_userList[cgmsg.actchairid].playstate = g_gdmjPlayerState.play_waiting
		if tItem.m_tInfo.beingpoker == 1 then
			--只有发牌的情况下会去检查流局
			isEnd = TdhService.CheckLiuJu(tItem)
		end
	end
...
function TdhService.DoPlay(tItem, cgmsg, gcmsg)
	--这个是打牌的操作
...
function TdhService.DoGuo(tItem,cgmsg,gcmsg)
...
function TdhService.DoPeng(tItem, cgmsg, gcmsg)
...
function TdhService.DoGang(tItem, cgmsg, gcmsg)
...
function TdhService.DoHu(tItem, cgmsg, gcmsg)
	--胡牌
end

function TdhService.DoQiangGangHu(tItem, cgmsg, gcmsg)
	
end

function TdhService.DoChiHu(tItem, cgmsg, gcmsg)
	
end

function TdhService.DoGenZhuang(tItem, cgmsg, gcmsg)
	
end

function TdhService.DoWin(tItem, pos, pokerID)
	--已经有人赢了
	tItem.m_tInfo.status = g_gdmjStatus.status_counting   --进入结算的时间
	tItem.m_tInfo.timemark = g_gdmjTime.counting_time
end

function TdhService.CheckLiuJu(tItem)
...
function TdhService.PlayCount(tItem)

	local len = #tItem.m_tInfo.winchairlist
	
	if #tItem.m_tInfo.winchairlist == 0 then
		TdhService.PlayCountLiuJu(tItem)
	elseif #tItem.m_tInfo.winchairlist == 1 then
		TdhService.PlayCountWin(tItem)
	else
		TdhService.PlayCountWinMore(tItem)
	end
...
function TdhService.PlayCountLiuJu(tItem)
...
function TdhService.PlayCountDissolve(tItem)
...
function TdhService.PlayCountWin(tItem)
	...
	elseif tItem.m_tInfo.wintype == g_gdmjAction.type_hu then  --这个是自摸胡
		...
		if tItem.m_vipRoomInfo.qiduijiafan == 1 then
			--如果是七对加番的，就必须检查胡的是否是七对了
			if 0 ~= GdmjHuHelper.IsQiDui(tItem, winChairID) then
				table.insert(desList,"七对加番")   --自摸
				doubleAccount = doubleAccount + 2
			end
		end
	...
	for i = 1,tItem.m_maxUser do
		...
		item.hunum = item.hunum + fenList[i]   --跟庄的也要顶上
	...
	--if tItem.m_tInfo.bankerpos == winChairID then
		--连庄的设置,这里首先要判断节节高
	if tItem.m_vipRoomInfo.jiejiegao ~= 0 then
		--只有节节高存在才会生效
...
function TdhService.PlayCountWinMore(tItem)
	local gcAccount = msg_gdmj_pb.gcgdmjaccount()
