...
function ZptdhService.DoWork(tItem) --传入的是TdhStruct类型的Item
	
	--print("status="..tItem.m_tInfo.status)
	
	if tItem.m_tInfo.status == g_gdmjStatus.status_ready then
		--牌桌处于等待的状态，如果人满了就开始
		ZptdhService.TableReady(tItem)
	...
	elseif tItem.m_tInfo.status == g_gdmjStatus.status_dissolve then
		...
		if tItem.m_tInfo.timemark <= 0 then
			--这里开始下发数据
			ZptdhService.PlayCountDissolve(tItem)
		end
...
function ZptdhService.TableReady(tItem)  --牌桌处于等待状态的函数
	local isReady = true
...
function ZptdhService.InitPublicPoker(tItem)
...
function ZptdhService.StartTest(tItem)
	...
	InsertTest(tItem, 1, 19)
	InsertTest(tItem, 1, 19)
	InsertTest(tItem, 1, 41)
	InsertTest(tItem, 1, 41)
	InsertTest(tItem, 1, 41)
	InsertTest(tItem, 1, 51)
	InsertTest(tItem, 1, 51)
	InsertTest(tItem, 1, 51)
...
function ZptdhService.GameStart(tItem)

	--初始化公共牌
	ZptdhService.InitPublicPoker(tItem)
...
function ZptdhService.TablePlay(tItem,currPos)
...
function ZptdhService.CheckMoving(tItem)
	
	local currPosList = GdmjWork.GetCurrPosList(tItem.m_tInfo)
	
	if #currPosList == 0 then
		LogFile("error", "ZptdhService.CheckMoving actchairid is nil")
		return nil
	end
...
function ZptdhService.PlayingAction(tItem,cgmsg, gcmsg)
	...
	if isWin == true then
		ZptdhService.DoWin(tItem, cgmsg.actchairid, cgmsg.actchairid)
		isEnd = true
	else
		tItem.m_userList[cgmsg.actchairid].playstate = g_gdmjPlayerState.play_waiting
		if tItem.m_tInfo.beingpoker == 1 then
			--只有发牌的情况下会去检查流局
			isEnd = ZptdhService.CheckLiuJu(tItem)
		end
	end
...
function ZptdhService.DoPlay(tItem, cgmsg, gcmsg)
	--这个是打牌的操作
...
function ZptdhService.DoGuo(tItem,cgmsg,gcmsg)
...
function ZptdhService.DoPeng(tItem, cgmsg, gcmsg)
...
function ZptdhService.DoGang(tItem, cgmsg, gcmsg)
...
function ZptdhService.DoHu(tItem, cgmsg, gcmsg)
	--胡牌
end

function ZptdhService.DoQiangGangHu(tItem, cgmsg, gcmsg)
	
end

function ZptdhService.DoChiHu(tItem, cgmsg, gcmsg)
	
end

function ZptdhService.DoGenZhuang(tItem, cgmsg, gcmsg)
	
end

function ZptdhService.DoWin(tItem, pos, pokerID)
	--已经有人赢了
	tItem.m_tInfo.status = g_gdmjStatus.status_counting   --进入结算的时间
	tItem.m_tInfo.timemark = g_gdmjTime.counting_time
end

function ZptdhService.CheckLiuJu(tItem)
...
function ZptdhService.PlayCount(tItem)

	local len = #tItem.m_tInfo.winchairlist
	
	if #tItem.m_tInfo.winchairlist == 0 then
		ZptdhService.PlayCountLiuJu(tItem)
	elseif #tItem.m_tInfo.winchairlist == 1 then
		ZptdhService.PlayCountWin(tItem)
	else
		ZptdhService.PlayCountWinMore(tItem)
	end
...
function ZptdhService.PlayCountLiuJu(tItem)
...
function ZptdhService.PlayCountDissolve(tItem)
...
function ZptdhService.PlayCountWin(tItem)
	...
	elseif tItem.m_tInfo.wintype == g_gdmjAction.type_hu then  --这个是自摸胡
		...
		if guiPubLen ~= 0 then
			if guiPriLen ~= 0 then
				...
				if tItem.m_vipRoomInfo.wuguijiabei == 1 then
					table.insert(desList,"无鬼加倍")   --自摸
					doubleAccount = doubleAccount + 2
				end
		...
		if tItem.m_vipRoomInfo.siguiliangbei == 1 then   --判断四鬼的
			if 0 ~= GdmjHuHelper.IsSiGui(tItem,winChairID) then
				table.insert(desList, "四鬼两倍")
				doubleAccount = 4--doubleAccount + 2
			end
		end
		
		if tItem.m_vipRoomInfo.pengpenghu == 1 then    --判断是否是碰碰胡
			if 0 ~= GdmjHuHelper.IsPengPengHu(tItem,winChairID) then
				table.insert(desList, "碰碰胡")
				doubleAccount = 4 --doubleAccount + 2
			end			
		end
		...
		if tItem.m_vipRoomInfo.qiduisibei == 1 then
			...
			if 0 ~= GdmjHuHelper.IsQiDui(tItem, winChairID) then
				table.insert(desList,"七对四倍")   --自摸
				doubleAccount = 8 --doubleAccount + 4
			end
		end
		...
		if tItem.m_vipRoomInfo.yaomeyao == 1 then
			
			if tItem.m_vipRoomInfo.yaojiujike == 1 then
				if 0 ~= GdmjHuHelper.IsHunYaoJiu(tItem, winChairID) then
					table.insert(desList,"幺九")
					doubleAccount = 12 --doubleAccount + 6
					print("yaojiu jike")
				end
			else
				if 0 ~= GdmjHuHelper.IsQingYaoJiu(tItem, winChairID) then
					table.insert(desList, "幺九")
					doubleAccount = 12 --doubleAccount + 6
					print("qing yaojiu")
				end
		...
		if tItem.m_vipRoomInfo.quanfeng == 1 then
			if 0 ~= GdmjHuHelper.IsQuanFeng(tItem, winChairID) then
				table.insert(desList, "全风")
				doubleAccount = 16 --doubleAccount + 8
			end
		end
		...
		for k1,v1 in ipairs(g_gdmjWinMa[maIndex]) do
			...
			for k2,v2 in ipairs(gcAccount.malist) do
				...
				if isMaGet == true then
					getMaNum = getMaNum + 1
					--doubleAccount = doubleAccount + 1  --每加一个码，就番一番
	...
	if tItem.m_tInfo.genzhuang ~= 0 then
		...
		for i =1,tItem.m_maxUser do
			if i == tItem.m_tInfo.bankerpos then
				fenList[i] = fenList[i] -2*(tItem.m_maxUser - 1)
				table.insert(desList,"跟庄")
			else
				fenList[i] = fenList[i] + 2   --鸡胡的分数
			end
		end
	end
...
function ZptdhService.PlayCountWinMore(tItem)
	local gcAccount = msg_gdmj_pb.gcgdmjaccount()
