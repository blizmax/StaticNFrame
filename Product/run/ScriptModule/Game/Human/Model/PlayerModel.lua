...
function PlayerModel.GetPlayerTableInfo(userID) --检查玩家是否在牌桌中
	...
	if g_servername == "run_hlyd" or g_servername == "run_zsmj" or g_servername == "run_gdmj" or g_servername == "run_xwpk" then
		tableID = HlydModel.GetUserTableID(userID)
		
		if tableID ~= nil and tonumber(tableID) == g_hlydDefine.table_common then
			--欢乐赢豆中，私人房不做房间恢复
			return g_logintable.type_hlyd,tonumber(tableID)
		end
		...
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			return g_logintable.type_sangong, tonumber(tableID)
		end
		...
		-- 牌九
		tableID = PaiJiuModel.GetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			return g_logintable.type_paijiu, tonumber(tableID)
		end
		tableID = DdzModel.GetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			return g_logintable.type_ddz, tonumber(tableID)
		end
		tableID = PdkModel.GetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			return g_logintable.type_ddz, tonumber(tableID)
		end
	end
	...
	if g_servername == "run_hubmj" then
		tableID = HubmjModel.GetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			return g_logintable.type_hubmj, tonumber(tableID)
		end
		
		tableID = SdbModel.SdbGetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			return g_logintable.type_hubmj, tonumber(tableID)
		end
		
		tableID = HgyxModel.HgyxGetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			return g_logintable.type_hubmj, tonumber(tableID)
		end
	...
	if g_servername == "run_ylmj" then
		...
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			return g_logintable.type_brnn, tonumber(tableID)
		end
...
function PlayerModel.GetCurrTableID(userID)
	...
	if g_servername == "run_hubmj" then
		tableID = HubmjModel.GetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			local tInfo = HubmjModel.GetTableInfo(tableID)
			if tInfo ~= nil then
				local userList = HubmjWork.GetUserList(tInfo, userID)
		...
		tableID = HgyxModel.HgyxGetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			local tInfo = HgyxModel.HgyxGetTableInfo(tableID)
			if tInfo ~= nil then
				local userList = HgyxService.GetUserIDList(tInfo,userID)
				return tableID, userList
			end
		end
		...
		tableID = SdbModel.SdbGetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			local tInfo = SdbModel.SdbGetTableInfo(tableID)
			if tInfo ~= nil then
				local userList = SdbService.GetUserIDList(tInfo, userID)
				return tableID, userList			
			end
		end
	end
	...
	--[[	HlydModel.GetUserTableID(userID)
			if tableID ~= nil and tonumber(tableID) ~= 0 then
				local tInfo = HlydModel.GetTableInfo(tableID)
				
				if tInfo ~= nil and userID == tInfo.bankerid then  --是房主才会发送离开的协议
					
					local userList = HlydService.GetUserOnlineList(tInfo, userID)
					
					SendMessage(userList,PacketCode[1024].client,gcmsg:ByteSize(), gcmsg:SerializeToString())
					return userID, 0, 0, gcmsg:SerializeToString()
				end
			end --]]
		local userTableList = HlydModel.GetUserTableList(userID)
		for k,v in ipairs(userTableList) do
			HlydModel.SetUserOffline(v,userID)
		end
		...
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			...
			if tInfo ~= nil then
				local userList = SanGongService.GetUserIDList(tInfo)
				return tableID, userList
			end
		end
	end
	...
	if g_servername == "run_zsmj" or g_servername == "run_xwpk" then
		...
		tableID = PaiJiuModel.GetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			local tInfo = PaiJiuModel.GetTableInfo(tableID)
			if tInfo ~= nil then
				local userList = PaiJiuModel.GetUserList(tInfo)
				return tableID, userList		
			end
		end
		...
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			...
			if tInfo ~= nil then
				local userList = DdzService.GetUserIDList(tInfo,exceptID)
				return tableID, userList		
			end
		end
	end
	...
	if g_servername == "run_ylmj" then
		...
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			...
			if tInfo ~= nil then
				local userList = BrnnService.GetUserIDList(tInfo)
				return tableID,userList
			end
		end
	end
	...
	if g_servername == "run_gdmj" then
		...
		tableID = PdkModel.GetUserTableID(userID)
		if tableID ~= nil and tonumber(tableID) ~= 0 then
			local tInfo = PdkModel.GetTableInfo(tableID)
			if tInfo ~= nil then
				local userList = PdkService.GetUserIDList(tInfo,exceptID)
				return tableID, userList		
			end
		end
	end
