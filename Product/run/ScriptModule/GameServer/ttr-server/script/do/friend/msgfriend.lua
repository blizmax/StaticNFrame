...
Net.CmdBeAskedPlayGame_C = function(cmd, laccount)
    ...
    if ask_userInfo ~= nil then
        ...
        ask_userInfo.mainTask:addProgress(TaskConditionEnum.AskFriendEvent, 1)
    end
...
Net.CmdSendReqAddFriendCmd_C = function(cmd, laccount)
    ...
    if userInfo ~= nil then
        ...
        userInfo.mainTask:addProgress(TaskConditionEnum.ApplyFriendEvent, 1)
    end
...
Net.CmdSendReqAgreeAddFriendCmd_C = function(cmd, laccount)
    ...
    if agree == true then
        ...
        local userInfo = UserInfo.GetUserInfoById(ask_uid)
        if userInfo ~= nil then
