...
--添加服务器定时器
function CPPNFrame:AddTimer(luaFunc, nInterVal, param_table)
...
--执行定时函数
function CPPNFrame.RunTimer(luaFunc, timerId, param_table)
    if param_table ~= nil and #param_table ~= 0 then
        CPPNFrame:BeginProfiler(luaFunc)
        CPPNFrame.RunStringFunction(luaFunc, table.unpack(param_table), timerId)
        local cost = CPPNFrame:EndProfiler()
        if cost >= 10000 then
...
function CPPNFrame:GetMD5(str)
    if type(str) ~= "string" then
        CPPNFrame:error("GetMD5 param error, not string:" .. str)
        return
    end
...
function CPPNFrame:GetCRC32(str)
    if type(str) ~= "string" then
        CPPNFrame:error("GetCRC32 param error, not string:" .. str)
        return
    end
...
function CPPNFrame:GetCRC16(str)
    if type(str) ~= "string" then
        CPPNFrame:error("GetCRC16 param error, not string:" .. str)
        return
    end
...
--执行加载函数
function CPPNFrame:load_script_file(  )
    for i, fun in ipairs(self.LoadScriptList) do
        fun()
    end
end

function CPPNFrame:InsertLoadFunc(func)
...
function CPPNFrame:debug(...)
	self.logModule:LuaDebug(...)
end

function CPPNFrame:info(...)
	self.logModule:LuaInfo(...)
end

function CPPNFrame:warn(...)
	self.logModule:LuaWarn(...)
end

function CPPNFrame:error(...)
...
function CPPNFrame.RunStringFunction(strFunction,...)
              ...
              for val in string.gmatch(w, "%[[\"%w_]+%]") do
                  ...
                  if value_str ~= nil then
                      ...
                      if value_num ~= nil then
                        ...
                        CPPNFrame:error("strFunction:", strFunction, " is not a function");
                      end
                  end
              end
          end
    ...
    if type(v) == "function" then
      return v(...);
        ...
        CPPNFrame:error(strFunction .. " is not function");
    end
  end
