...
function unilight.init(pluginManager)
	...
	TcpServer.addRecvCallBack(NF_SERVER_TYPES.NF_ST_GAME, 0, "unilight.NetServerRecvHandleJson")
	TcpClient.addRecvCallBack(NF_SERVER_TYPES.NF_ST_GAME, 0, "unilight.NetClientRecvHandleJson")
end
...
--特殊协议
function unilight.NetClientRecvHandleJson(unLinkId, valueId, nMsgId, strMsg)
    unilight.debug("unLinkId:" .. unLinkId .. " valueId:" .. valueId .. " nMsgId:" .. nMsgId .. " strMsg:" .. strMsg)
    local table_msg = json2table(strMsg)
    --协议规则
    if table_msg ~= nil then
        ...
        if type(cmd) == "string" then
            ...
            if strcmd ~= "" then
				...
				if type(Net[strcmd]) == "function" then
					...
					laccount.Id = valueId
					laccount.unLinkId = unLinkId
					laccount.SendString = TcpSClient.sendJsonMsg(strMsg, la)
