...
#ifndef __CTLIB_OBJECTMGR_H__
#define __CTLIB_OBJECTMGR_H__
...
public:
    ...
    */
    ...
    public:
        ...
        CTListInfo() : iHead(-1), iTail(-1), iSize(0) {}
    ...
    */
    ...
    public:
        ...
        CTList() : iPrev(-1), iNext(-1), iFlag(0) {}
...
protected:
    ...
    */
    ...
    {
    	CT_ASSERT(!(!pBuffer || iObjectCount <= 0 || iObjectSize <= 0 || iListCount < 2 || iIndexCount < 1));
        ...
        m_pstInfo = (CTListInfo*) (pBuffer);
        m_pstList = (CTList*) (pBuffer + sizeof(CTListInfo) * m_iListCount * m_iIndexCount);
        m_pDataBuffer = (char*) pBuffer + sizeof(CTListInfo) * m_iListCount * m_iIndexCount
                    + sizeof(CTList) * m_iObjectCount * m_iIndexCount;
...
public:
    ...
    */
    ...
    {
        return sizeof(CTListInfo) * iListCount * iIndexCount
             + sizeof(CTList) * iObjectCount * iIndexCount
...
public:
    ...
    * 链表的ListID就是iListID
    */
    void BuildInfoList(CTListInfo& stListInfo, int iListID, int iIndexID = 0)
    {
    ...
    */
    int InfoListAlloc(int iIndexID, int iNewListID)
    {
        ...
        {
            CT_ERROR(("InfoListAlloc error : invalid index id(%d) or new list id(%d)", iIndexID, iNewListID));
            return -1;
        }
    ...
    */
    int InfoListMove(int i, int iIndexID, int iNewListID)
    {
        if (iIndexID < 0 || iIndexID >= m_iIndexCount || iNewListID < 0 || iNewListID >= m_iListCount)
        {
            CT_ERROR(("InfoListMove error : invalid index id(%d) or new list id(%d)", iIndexID, iNewListID));
            return -1;
        }
...
private:
    ...
    */
    void InfoListRemove(CTListInfo& stListInfo, int i, int iIndexID)
    {
    ...
    * @note stListInfo从外部传入，链表头尾的存放可以更灵活
    */
    void InfoListAppend(CTListInfo& stListInfo, int i, int iIndexID)
    {
    ...
    * @note 如果已在这号链表中 会加入到链表的最后
    */
    int InfoListMove(int i, int iIndexID, int iNewListID, CTListInfo& stOldList, CTListInfo& stNewList)
    {
        ...
        {
            CT_ERROR(("index(%d) list is %d, index id is %d, not in old list", i, Flag(i, iIndexID) + iIndexID * m_iListCount, iIndexID));
            return -4;
        }
        ...
        {
            CT_ERROR(("InfoListMove error, flag changed"));
        }

        if (Next(i, iIndexID) != -1)
        {
            CT_ERROR(("InfoListMove error, next is not -1"));
        }
        ...
        if (Prev(i, iIndexID) == -1) //头节点
        {
            ...
            {
                CT_ERROR(("InfoListMove error, head error"));
            }
        }
        else
        {
           ...
           {
               CT_ERROR(("InfoListMove error, prev is not in this list"));
           }
        }
...
public:
    ...
    * @return 成功就返回分配元素的数组下标，失败时返回-1
    */
    int InfoListAlloc(int iIndexID, int iNewListID, CTListInfo& stOldList, CTListInfo& stNewList)
    {
        if (stOldList.iSize <= 0)
        {
            return -1;
...
public:
    ...
    // ListInfo 操作
    const CTListInfo& ListInfo(int iListID, int iIndexID = 0) const
    {
        return m_pstInfo[iIndexID * m_iListCount + iListID];
    }

    CTListInfo& ListInfo(int iListID, int iIndexID = 0)
    {
        CT_ASSERT(iIndexID * m_iListCount + iListID < m_iIndexCount * m_iListCount);
        return m_pstInfo[iIndexID * m_iListCount + iListID];
    }

    const CTList& List(int i, int iIndexID = 0) const
    {
    ...
    CTList& List(int i, int iIndexID = 0)
    {
        return m_pstList[iIndexID * m_iObjectCount + i];
    }
...
protected:
    ...
    CTListInfo*		m_pstInfo;
    CTList*			m_pstList;
    char*			m_pDataBuffer;
};

template<typename T>
class CTObjectMgr : public CTObjectMgrBase
{
public:
    virtual ~CTObjectMgr() {}
    ...
    {
        int iRet = CTObjectMgrBase::Init(pBuffer, iObjectCount, iObjectSize, iListCount, iIndexCount, bResetShm);
        if (iRet)
        {
            return iRet;
        }
        ...
        {
            bool bIsReset = OBJ_STAT->IsReset();
...
template<>
class CTObjectMgr<char*> : public CTObjectMgrBase
{
public:
    virtual ~CTObjectMgr() {}
