...
#define NF_NEW new
#define NFCountArray(Array) (sizeof(Array)/sizeof(Array[0]))

#define  NFSafeRelease(pObject) { if (pObject!=NULL) { pObject->Release(); pObject=NULL; } }

#define  NFSafeDelete(pData) { try { delete pData; } catch (...) { NFASSERT(false, "NFSafeDelete error", __FILE__, __FUNCTION__); } pData=NULL; }

#define  NFSafeDeleteArray(pData) { try { delete [] pData; } catch (...) {  NFASSERT(false, "NFSafeDeleteArray error", __FILE__, __FUNCTION__); } pData=NULL; }
