...
* @brief Hash map异常类
*/
struct NF_HashMap_Exception : public NFException
{
	NF_HashMap_Exception(const std::string &buffer) : NFException(buffer) {};
	NF_HashMap_Exception(const std::string &buffer, int err) : NFException(buffer, err) {};
	~NF_HashMap_Exception() throw() {};
};
...
*
*该hashmap通过NFMemMutilChunkAllocator来分配空间，支持不同大小的内存块的分配；
*
...
#pragma pack(pop)
		...
		*          其他异常
		*/
		int getBlockData(NFMemHashMap::BlockData &data);
		...
		* @param vtData, 淘汰的数据
		*/
		int set(const void *pData, size_t iDataLen, bool bOnlyKey, std::vector<NFMemHashMap::BlockData> &vtData);
	...
	protected:
		...
		* @return int,
		*/
		int allocate(size_t iDataLen, std::vector<NFMemHashMap::BlockData> &vtData);
		...
		* @return int
		*/
		int allocateChunk(size_t fn, std::vector<size_t> &chunks, std::vector<NFMemHashMap::BlockData> &vtData);
	...
	*/
	...
	public:
		...
		*/
		size_t allocateMemBlock(size_t index, size_t &iAllocSize, std::vector<NFMemHashMap::BlockData> &vtData);
	...
	*/
	...
	protected:
		...
		* @return int
		*/
		int set(const std::string& k, const std::string& v, std::vector<NFMemHashMap::BlockData> &vtData);
		...
		* @return int
		*/
		int set(const std::string& k, std::vector<NFMemHashMap::BlockData> &vtData);
	...
	*/
	...
	public:
		...
		* @return
		*/
		void get(std::vector<NFMemHashMap::BlockData> &vtData);
...
protected:
	...
	//禁止copy构造
	NFMemHashMap(const NFMemHashMap &mcm);
	//禁止复制
	NFMemHashMap &operator=(const NFMemHashMap &mcm);
