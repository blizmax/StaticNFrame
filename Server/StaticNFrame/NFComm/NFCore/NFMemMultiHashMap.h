...
*/
struct NF_Multi_HashMap_Exception : public NFException
{
	NF_Multi_HashMap_Exception(const std::string &buffer) : NFException(buffer) {};
	NF_Multi_HashMap_Exception(const std::string &buffer, int err) : NFException(buffer, err) {};
	~NF_Multi_HashMap_Exception() throw() {};
};
...
#pragma pack(pop)
		...
		* @param iAddr, 主key的地址
		*/
		MainKey(NFMemMultiHashMap *pMap, uint32_t iAddr)
			: _pMap(pMap)
			, _iHead(iAddr)
		{
			// 构造时保存绝对地址，避免每次都计算
		...
		* @param vtData, 淘汰的数据
		*/
		int set(const void *pData, uint32_t iDataLen, std::vector<NFMemMultiHashMap::Value> &vtData);
	...
	protected:
		...
		* @return int
		*/
		int allocate(uint32_t iDataLen, std::vector<NFMemMultiHashMap::Value> &vtData);
		...
		* @return int
		*/
		int allocateChunk(uint32_t fn, std::vector<uint32_t> &chunks, std::vector<NFMemMultiHashMap::Value> &vtData);
	...
	public:
		bool ISFULLDATA() { return NFMemMultiHashMap::ISSET(getHeadPtr()->_iBitset, INTEGRITY_BIT); }
	protected:
		bool HASNEXTCHUNK() { return NFMemMultiHashMap::ISSET(getHeadPtr()->_iBitset, NEXTCHUNK_BIT); }

	public:
		void SETFULLDATA(bool b) { if (b) NFMemMultiHashMap::SET(getHeadPtr()->_iBitset, INTEGRITY_BIT); else NFMemMultiHashMap::UNSET(getHeadPtr()->_iBitset, INTEGRITY_BIT); }
	protected:
		void SETNEXTCHUNK(bool b) { if (b) NFMemMultiHashMap::SET(getHeadPtr()->_iBitset, NEXTCHUNK_BIT); else NFMemMultiHashMap::UNSET(getHeadPtr()->_iBitset, NEXTCHUNK_BIT); }
...
#pragma pack(pop)
		...
		* @param pAdd
		*/
		Block(NFMemMultiHashMap *pMap, uint32_t iAddr)
			: _pMap(pMap)
			, _iHead(iAddr)
		{
			// 构造时保存绝对地址，避免每次都计算
		...
		*          其他异常
		*/
		int getBlockData(NFMemMultiHashMap::BlockData &data);
		...
		*                其它为失败
		*/
		int set(const void *pData, uint32_t iDataLen, bool bOnlyKey, uint8_t iVersion, std::vector<NFMemMultiHashMap::Value> &vtData);
	...
	protected:
		...
		*/
		...
		bool HASNEXTCHUNK() { return NFMemMultiHashMap::ISSET(getBlockHead()->_iBitset, NEXTCHUNK_BIT); }
		bool ISDIRTY() { return NFMemMultiHashMap::ISSET(getBlockHead()->_iBitset, DIRTY_BIT); }
		bool ISONLYKEY() { return NFMemMultiHashMap::ISSET(getBlockHead()->_iBitset, ONLYKEY_BIT); }

		void SETNEXTCHUNK(bool b) { if (b) NFMemMultiHashMap::SET(getBlockHead()->_iBitset, NEXTCHUNK_BIT); else NFMemMultiHashMap::UNSET(getBlockHead()->_iBitset, NEXTCHUNK_BIT); }
		void SETDIRTY(bool b) { if (b) NFMemMultiHashMap::SET(getBlockHead()->_iBitset, DIRTY_BIT); else NFMemMultiHashMap::UNSET(getBlockHead()->_iBitset, DIRTY_BIT); }
		void SETONLYKEY(bool b) { if (b) NFMemMultiHashMap::SET(getBlockHead()->_iBitset, ONLYKEY_BIT); else NFMemMultiHashMap::UNSET(getBlockHead()->_iBitset, ONLYKEY_BIT); }
	...
	*/
	...
	public:
		...
		* @brief 构造函数
		*/
		BlockAllocator(NFMemMultiHashMap *pMap)
			: _pMap(pMap)
		...
		*/
		uint32_t allocateMemBlock(uint32_t iMainKeyAddr, uint32_t index, bool bHead, uint32_t &iAllocSize, std::vector<NFMemMultiHashMap::Value> &vtData);
		...
		*/
		uint32_t allocateMainKeyHead(uint32_t index, std::vector<NFMemMultiHashMap::Value> &vtData);
		...
		*/
		uint32_t allocateChunk(uint32_t iAddr, uint32_t &iAllocSize, std::vector<NFMemMultiHashMap::Value> &vtData);
	...
	*/
	...
	public:
		...
		*/
		HashMapLockItem(NFMemMultiHashMap *pMap, uint32_t iAddr);
		...
		*
		*/
		int get(NFMemMultiHashMap::Value &v);
	...
	protected:
		...
		* @return int
		*/
		int set(const std::string &mk, const std::string &uk, const std::string& v, uint8_t iVersion, std::vector<NFMemMultiHashMap::Value> &vtData);
		...
		* @return int
		*/
		int set(const std::string &mk, const std::string &uk, std::vector<NFMemMultiHashMap::Value> &vtData);
		...
		* @return bool
		*/
		bool equal(const std::string &mk, const std::string &uk, NFMemMultiHashMap::Value &v, int &ret);
		...
		*/
		...
		friend class NFMemMultiHashMap;
		friend struct NFMemMultiHashMap::HashMapLockIterator;
	...
	*/
	...
	public:
		...
		* @param iOrder, 遍历的顺序
		*/
		HashMapLockIterator(NFMemMultiHashMap *pMap, uint32_t iAddr, int iType, int iOrder);
	...
	*/
	...
	public:
		...
		* @param iIndex, Hash索引
		*/
		HashMapItem(NFMemMultiHashMap *pMap, uint32_t iIndex);
		...
		* @return
		*/
		void get(std::vector<NFMemMultiHashMap::Value> &vtData);
		...
		*/
		...
		friend class NFMemMultiHashMap;
		friend struct NFMemMultiHashMap::HashMapIterator;
	...
	*/
	...
	public:
		...
		* @param type
		*/
		HashMapIterator(NFMemMultiHashMap *pMap, uint32_t iIndex);
...
#pragma pack(pop)
	...
	*/
	NFMemMultiHashMap()
		...
		, _hashf(NFHash::magic_string_hash())
	{
	}
...
protected:
	...
	*  @brief 禁止copy构造
	*/
	NFMemMultiHashMap(const NFMemMultiHashMap &mcm);
	/**
	*  @brief 禁止复制
	*/
	NFMemMultiHashMap &operator=(const NFMemMultiHashMap &mcm);
	...
	*/
	...
	{
		FailureRecover(NFMemMultiHashMap *pMap) : _pMap(pMap)
		{
			// 构造时恢复可能损坏的数据
			_pMap->doRecover();
			assert(_iRefCount++ == 0);
...
protected:
	...
	* 主key hash索引区
	*/
	NFMemVector<tagMainKeyHashItem>    _hashMainKey;
