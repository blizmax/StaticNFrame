...
protected:
	...
	bool OnNodeCallback(const uint32_t index, const NFCData& oldData, const NFCData& newData);
private:
	std::vector<NFDataNode*> mNodes;
    std::unordered_map<std::string, uint32_t> mIndices;
	std::unordered_map<uint32_t, std::vector<DATA_NODE_EVENT_FUNCTOR_PTR>> mCallBackList;
    uint64_t mObjectId;
};
